...
class OpenIDHandler extends Handler
{
	...
	function doAuthentication($args, $request)
	{
		...
		// Sync localized name fields AFTER registerUserSession().
		// Why: registerUserSession() calls Repo::user()->edit($user) which overwrites
		// name fields with old values. And edit()'s schema sanitizer strips multilingual
		// fields from update params. Writing directly to user_settings after all edit()
		// calls ensures the provider's claims persist.
		$settings = OpenIDPlugin::getOpenIDSettings($this->plugin, $contextId);
		if (($settings['providerSync'] ?? false) && $userClaims !== null) {
			$locale = $contextData->getPrimaryLocale();
			$userId = $user->getId();

			if (!empty($userClaims->givenName)) {
				\Illuminate\Support\Facades\DB::table('user_settings')->updateOrInsert(
					['user_id' => $userId, 'setting_name' => 'givenName', 'locale' => $locale],
					['setting_value' => $userClaims->givenName]
				);
			}
			if (!empty($userClaims->familyName)) {
				\Illuminate\Support\Facades\DB::table('user_settings')->updateOrInsert(
					['user_id' => $userId, 'setting_name' => 'familyName', 'locale' => $locale],
					['setting_value' => $userClaims->familyName]
				);
			}
		}
		...
		if ($this->plugin->isEnabledSitewide() && $user->hasRole([Role::ROLE_ID_SITE_ADMIN], \PKP\core\PKPApplication::CONTEXT_SITE)) {
	...
	): void
	{
		...
		// Sync non-localized fields via the standard edit() pipeline
		...
		// Note: localized name fields (givenName, familyName) are synced in doAuthentication()
		// AFTER registerUserSession(), because registerUserSession calls edit($user) which
		// overwrites any changes made here.
	}
